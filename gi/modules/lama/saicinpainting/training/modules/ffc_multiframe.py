...
class FFCResNetGeneratorMultiframe(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_downsampling=3, n_blocks=9, norm_layer=nn.BatchNorm2d,
        ...
        for i in range(n_blocks):
            ...
            model += [cur_resblock]
            if i == n_blocks//2:
                self.encoder = [module for module in model]
        ...
        self.decoder = model[len(self.encoder):]
        self.model = nn.Sequential(*model)

    def forward(self, input):
        btchw = input.shape
        h = input.reshape(btchw[0]*btchw[1],btchw[2],btchw[3],btchw[4])
        for module in self.encoder:
            h = module(h)
        # TODO do good with it
        assert type(h)==tuple
        h = tuple(rearrange(subh, "(b t) c h w -> b t c h w", t=btchw[1]) for subh in h)
        hcorr = torch.cat(h, dim=2)
        hcorr = self.correlate(hcorr) # should return bchw
        h = tuple(subh[:,0,...] for subh in h)
        hdims = list(subh.shape[1] for subh in h)
        hcorr = torch.split(hcorr, hdims, dim=1)
        h = tuple(subh+subhcorr for subh, subhcorr in zip(h, hcorr))
        for module in self.decoder:
            h = module(h)
        return h

    def correlate(self, hcorr):
        # receives btchw and should return features aggregated for for t=0 in
        # shape bchw
